import { pgTable, serial, date, varchar, integer, timestamp, boolean, text } from 'drizzle-orm/pg-core';
...
export async function handleLogin({ profile }: any) {
  try {
    const queryClient = postgres(`postgres://postgres:${process.env.DB_PASSWORD ?? ""}@0.0.0.0:5432/KnowX`);
    const db = drizzle(queryClient);
    
    const uniqueUser: User[] = await db.select().from(user).where(eq(user.email, profile.email));
    ...
    await logError({ userId: -1, description: String(error), origin: "handleLogin" });
    return false;
  }
}

export async function logSearch({ userId, search, feedback }: { userId: number, search: string, feedback: boolean }) {
  try {
    const queryClient = postgres(`postgres://postgres:${process.env.DB_PASSWORD ?? ""}@0.0.0.0:5432/KnowX`);
    const db = drizzle(queryClient);
...
async function logError({ userId, description, origin }: { userId: number, description: string, origin: string }) {
  const queryClient = postgres(`postgres://postgres:${process.env.DB_PASSWORD ?? ""}@0.0.0.0:5432/KnowX`);
  const db = drizzle(queryClient);
